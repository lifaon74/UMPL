...
/*VString.prototype.fromBuffer = function(isFromBuffer) {
	if(_.isBool(isFromBuffer)) {
		this._fromBuffer = isFromBuffer;
		return this;
	} else {
		if(_.isBool(isFromBuffer)) {	
			return this._fromBuffer;
		} else {
			return false;
		}
	}
...
UMPLCompiler.compileFile = function(inputFilePath, outputFilePath, verbose) {
	var compiler = new UMPLCompiler();
	
	if(typeof verbose != "boolean") { var verbose = false; }
	
	var compiledFolder	= "compiled/" + inputFilePath.replace(/\\|\//g, '_');
	var compiler = new UMPLCompiler();
	...
	compiler.bind('parse_loop', function(jsCode) {
		if(verbose) {
			fs.writeFileSync(compiledFolder + '/out_' + this.loop + '.js', this.jsCode.toString(), 'utf8');
		}
	});

	compiler.bind('execute_loop', function(code) {
		if(verbose) {
			fs.writeFileSync(compiledFolder + '/out_' + this.loop + '.txt', this.code.toString(), 'utf8');
		}
	});

	compiler.bind('compiled', function(outputCode) {
		console.log('Final code in ' + outputFilePath);
...
UMPLCompiler.prototype.compile = function(inputCode, config) {
	...
	if(!self.compiling) {
		...
		self.globals	= {};
		self.scope['$globals'] = self.globals;
			
		self.jsCode		= new VString();
		self.loop		= 0;
		self.level		= 0;
		
		self.safeExecution = (typeof config.safeExecution == 'boolean') ? config.safeExecution : true;
...
UMPLCompiler.prototype.compileFile = function(inputFile, config) {
	try {
		var inputCode = fs.readFileSync(inputFile, 'utf8');
	} catch(e) {
		throw {
			code: "file_not_found",
			message: "file " + inputFile + " not found"
		};
	}
	
	this.compile(inputCode, config);
};
...
UMPLCompiler.prototype.compileLoop = function() {
	...
	while(true) {
		try {
			...
			error.type = "parsing";
			self._on_compile_error(error, self.code, self.loop);
		...
		if(self.jsCode === null) {
			...
			try {
				...
				if(error instanceof SyntaxError) {
					...
					self._on_compile_error(error, self.jsCode, self.loop);
					self.errorOccured = true;
				}
			}
		}
				...
				if(open_token.match.end.variables[0] == "=") {  // =%> txt [tag] js [tag]
					if(close_token.match.start.variables[1] == "=") {  // =%> txt <%= js [tag]
						if(close_token.match.end.variables[0] == "=") { // =%> txt <%= js =%>
							...
							throw {
								code:		"invalid_tags_chain",
								message:	"you can't do =%> tx t<%= js %> , the last close tag must be =%> in this situation",
								position:	close_token.match.end.position.start,
								token: close_token
							};
						}
					} else { // =%> txt <% js [tag]
...
UMPLCompiler.prototype.execute = function(code) {
	...
	try {
		switch(self.execMode) {
			...
			case self.execModes.REQUIRE:
				...
				var jsCode = "module.exports.run = function($scope) {" + scopeVars + code + " };"
		...
		if(error instanceof SyntaxError) {
			...
			if(matches.length > 0) {
				...
				line	= parseInt(matchLineColumn.variables[0]) - 2;
	...
	UMPLCompiler.prototype._on_compile_error = function(error, vstring, loop) {
		var self = this;
		
		if(typeof error.type != "undefined") {
			//var extension = "txt";
			
			error.associatedCode	= vstring;
			error.loop				= loop;
			error.formatedString	= "";
			
			switch(error.type) {
				case "parsing":
					error.formatedString += "[ERROR (COMPILATION)] ";
					...
					for(var i = 0; i < error.positions.length; i++) {
						...
						error.formatedString += UMPLCompiler.indexToLineColumnString(vstring.get(), error.positions[i]);
					}
				...
				case "executing":
					if(typeof error.line == "undefined") {
						...
						error.formatedString	+= "[ERROR (EXECUTION)] ";
						error.formatedString	+= UMPLCompiler.lineColumnToString(error.line, error.column);
						//extension	= "js";
					}
				break;
			}
