...
UMPLCompiler.prototype.compile = function(inputCode, config) {
	...
	if(!self.compiling) {
		...
		self.compiling 	= true;
		self.code		= new $Buffer();
		self.global		= {};
			self.code.write(inputCode);
		self.jsCode		= "";
		self.loop		= 0;
		self.level		= 0;
		...
		if(!self.errorOccured) {
			self.trigger('compiled', [self.code.buffer]);
		}
...
UMPLCompiler.prototype.compileLoop = function() {
	...
	while(true) {
		try {
			self.jsCode = self.convertToJS(self.code.buffer);
		} catch(error) {
			error.type = "parsing";
			self._on_compile_error(error, self.code.buffer, self.loop);
		...
		UMPLCompiler.prototype._parseTokens = function(visibilityString, tokens) {
			...
			for(var i = 0; i < tokens.length; i++) {
				...
				code += self._bufferWrite(
					visibilityString.slice(lastIndex, token.start, VisibilityString.transparent)
				);
			...
			code += self._bufferWrite(
				visibilityString.slice(lastIndex, visibilityString.string.length, VisibilityString.transparent)
			);
	...
	try {
		switch(self.execMode) {
			case self.execModes.EVAL:
				(new Function('$compiler', '$buffer', 'require', code))(self, self.$buffer, require);
			break;
			...
			break;
			case self.execModes.REQUIRE:
				var jsCode = "module.exports.run = function($compiler, $buffer, require) {" + code + " };"
				fs.writeFileSync(filePath, jsCode);
				var debug = require(filePath);
				fs.unlinkSync(filePath);
				debug.run(self, self.$buffer, require);
			break;
		}
