...
UMPLCompiler.prototype.compile = function(inputCode, config) {
	...
	if(!self.compiling) {
		...
		if(!self.errorOccured) {
			self.trigger('compiled', [self.code.get()]);
		}
...
UMPLCompiler.prototype.compileLoop = function() {
	...
	while(true) {
		try {
			self.jsCode = self.convertToJS(self.code.get());
		} catch(error) {
			error.type = "parsing";
			self._on_compile_error(error, self.code.get(), self.loop);
...
UMPLCompiler.prototype.execute = function(code) {
	...
	UMPLCompiler.prototype._on_compile_error = function(error, string, loop) {
		var self = this;
		
		if(typeof error.type != "undefined") {
			//var extension = "txt";
			
			error.associatedCode	= string;
			error.loop				= loop;
